...
command -v random >/dev/null 2>&1 || die "the random CLI is required"
mkdir -p "$CACHE"
...
[ -f "$CACHE/blob_a.bin" ] || random -b -c "$A_BYTES" --seed 0x700A > "$CACHE/blob_a.bin"
[ -f "$CACHE/blob_b.bin" ] || random -b -c "$B_BYTES" --seed 0x700B > "$CACHE/blob_b.bin"
...
unrar t "$CACHE/v70_longrange.rar" >/dev/null 2>&1 || die "unrar refuses the corpus"

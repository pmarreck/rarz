...
| RAR3 PPMd text compression | Strict | The PPMd decoder was a non-functional stand-in until 2026-08-27 (model never learned; init misread the flag byte) — an instant false positive on one stream shape, minutes of escape churn on another, exposed by a 1115 MB real-world witness. Rewritten clean-room from the reference (model.cpp/suballoc.cpp/coder.cpp). Fixtures decode byte-identical to unrar; 12/12 mutations refused; the witness verifies all 1539 entries in 23.9 s wall vs unrar 39.1 s. CAVEAT: a PPM block allocates its own heap, up to 256 MiB, declared INSIDE the compressed stream — not visible to `rarz_max_dictionary_size()` pre-decode. |
...
| RAR5/7 / UnpVer 70 | Strict (local corpus gate) | CONFIRMED against the first positively-identified v70 stream: a deterministic 7.07 GB A-B-A corpus (drandomz seeds 0x700A/0x700B, generate_v70_corpus.sh) that rar 7.20 encodes at -md=6784m — v70 marker, 8 GiB window, >4 GiB match distances, and a FRACTIONAL dictionary in one artifact. Three latent defects fixed on the way: is_rar7 was dead code (algo_version raw 0/1 compared against 70), the dict exponent parse was version-blind (v50 masks 4 bits, v70 masks 5 + fraction at shift 15, FCI_RAR5_COMPAT honored), and distances were u32. Windows allocate at next power of two (safe with monotonic positions; how fractional sizes decode without hot-path modulo); dictionaries above 8 GiB now REFUSE as unsupported instead of silently clamping (GH #17). Gate skips honestly on machines without the ~11 GiB cache. |
...
| Entries larger than the dictionary | Strict, all decoders | unpack29 (2026-08-05), unpack20 (2026-08-06), unpack50 (2026-08-27) all stream entries through the window now. Verification peak memory = max dictionary + fixed state, independent of entry size; `rarz_max_dictionary_size()` exposes the budget pre-decode. Witnessed: a 1.39 GB entry at 43x its window verified in 2.04 s / 65.5 MB peak RSS, mutations refused. |
...
| Header encryption (`-hp`) | Not covered, truthfully reported | Detected pre-walk (RAR5 HEAD_CRYPT block; RAR4 MHD_PASSWORD on MAIN). Was FALSE DAMAGE until 2026-08-27: ciphertext was CRC-checked as headers ("header CRC mismatch", INVALID) and the C summary said VERIFIED over zero entries. Now: `t` prints UNVERIFIABLE + an Info line, `verify` and `t` exit 69, the summary is INCOMPLETE, and `rarz_header_encrypted()` exposes the fact. Gate: test_verify.sh Gate G over committed rar4_hp/rar5_hp fixtures. |
